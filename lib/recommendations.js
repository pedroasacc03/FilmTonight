...
Suggest ${movieCount} movies and ${tvCount} TV shows now.`;
  ...
  const { data: rawCandidates, usage } = await askClaudeForJSON({ system, prompt, maxTokens: 1500, model: getProfileModel() });
...
Suggest one surprising-but-thoughtful pick now.`;
    ...
    const { data: candidate, usage } = await askClaudeForJSON({ system, prompt, maxTokens: 500, model: getProfileModel() });
