...
import { getClaudeClient, getProfileModel, extractText, sumUsage } from "@/lib/anthropic";
...
patterns instead of just going by genre.`;
  ...
  // One user message can take several Claude calls (one per tool-loop
  // iteration) - accumulated so the cost event fired below reflects the
  // full cost of this one chat turn, not just its last API call.
  const usageList = [];
  ...
  await trackCostEvent(userId, EVENT_NAMES.COST_CHAT_MESSAGE, sumUsage(usageList));
