...
export async function createProUpgradeIntent({ userId, email, source, billingCycleSelected }) {
  ...
  if (existing) {
    return {
      granted: existing.grantedBetaPro,
      waitlisted: !existing.grantedBetaPro,
      alreadyRecorded: true,
      // The real daily number, included so the client can state it plainly
      // instead of hiding it - see components/EnergyLimitWatcher.js.
      ceiling: getEnergyCeiling(existing.grantedBetaPro ? "beta_pro" : "free"),
    };
  }
  ...
  return { granted, waitlisted: !granted, alreadyRecorded: false, ceiling: getEnergyCeiling(granted ? "beta_pro" : "free") };
}
