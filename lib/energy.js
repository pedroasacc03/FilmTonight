...
export const FREE_DAILY_ENERGY = 6; // worst case ~6 x $0.0093 x 30 =~ $1.46/mo/user (a fully mixed-day)
export const PRO_DAILY_ENERGY = 7; // ~3 rec batches + 4 chats/day => worst case ~$2.41/mo/user at $5/mo (~47% margin)
...
// midnight. The `where: { lastRefillAt: { lt: todayStart } }` clause (not a
// blind write) is what makes this race-safe: if two requests both see a
// stale row and both attempt this update, only the first one's write
// actually matches the condition at the moment it runs (lastRefillAt is now
// `new Date()`, which is >= todayStart) - the second becomes a no-op
// (count: 0) instead of refilling twice. Same atomic-conditional-update
// pattern as the spend check below, applied to the same race shape.
async function refillIfStale(userId, tier) {
  const now = new Date();
  const todayStart = startOfUtcDay(now);
  await prisma.userEnergy.updateMany({
    where: { userId, lastRefillAt: { lt: todayStart } },
    data: { energy: getEnergyCeiling(tier), lastRefillAt: now },
  });
}
...
export async function getEnergyStatus(userId, tier) {
  ...
  return {
    tier,
    energy: Math.min(row.energy, ceiling), // clamp defensively - see refundEnergy's comment on why this can't go negative but could in principle overshoot
    ceiling,
 ...
 * { allowed: false, energyRemaining, resetAt, tier, action }, the last shape
 * matching the 429 body every metered route returns verbatim.
 */
export async function checkAndSpendEnergy(userId, tier, action) {
  ...
  return { allowed, energyRemaining: row.energy, resetAt, tier, action };
}
